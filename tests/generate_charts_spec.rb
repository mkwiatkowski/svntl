...
context "generate_charts method for existing repository" do
  ...
  specify "should create named directory if `directory` argument was passed" do 
    ...
    generate_charts 'file:///existing/repository', :directory => 'svntl'
  end
  ...
  specify "should call chart generation methods with `file` argument equal to `dir\\chart.png` in systems which has backslash as directory separator" do
    ...
    @repository_object.should_receive(:chart_loc_per_day).once.with(:file => 'dir\loc_per_day.png')
    @repository_object.should_receive(:chart_loc_per_day).once.with(:file => 'dir\loc_per_day_small.png', :small => true)
    @repository_object.should_receive(:chart_loc_per_commit).once.with(:file => 'dir\loc_per_commit.png')
    @repository_object.should_receive(:chart_loc_per_commit).once.with(:file => 'dir\loc_per_commit_small.png', :small => true)
